package com.example.novel_summary.data.repository


import com.example.novel_summary.data.dao.NovelDao
import com.example.novel_summary.data.dao.VolumeDao
import com.example.novel_summary.data.dao.ChapterDao
import com.example.novel_summary.data.model.Novel
import com.example.novel_summary.data.model.Volume
import com.example.novel_summary.data.model.Chapter
import kotlinx.coroutines.flow.Flow

class NovelRepository(
    private val novelDao: NovelDao,
    private val volumeDao: VolumeDao,
    private val chapterDao: ChapterDao
) {

    val allNovels: Flow<List<Novel>> = novelDao.getAllNovels()

    suspend fun insertNovel(novel: Novel): Long {
        return novelDao.insertNovel(novel)
    }

    suspend fun updateNovel(novel: Novel) {
        novelDao.updateNovel(novel)
    }

    suspend fun deleteNovel(novel: Novel) {
        novelDao.deleteNovel(novel)
    }

    suspend fun getNovelById(id: Long): Novel? {
        return novelDao.getNovelById(id)
    }

    suspend fun getNovelByName(name: String): Novel? {
        return novelDao.getNovelByName(name)
    }

    // Volume operations
    fun getVolumesByNovelId(novelId: Long): Flow<List<Volume>> {
        return volumeDao.getVolumesByNovelId(novelId)
    }

    suspend fun insertVolume(volume: Volume): Long {
        return volumeDao.insertVolume(volume)
    }

    suspend fun updateVolume(volume: Volume) {
        volumeDao.updateVolume(volume)
    }

    suspend fun deleteVolume(volume: Volume) {
        volumeDao.deleteVolume(volume)
    }

    suspend fun getVolumeById(id: Long): Volume? {
        return volumeDao.getVolumeById(id)
    }

    suspend fun getVolumeByName(novelId: Long, volumeName: String): Volume? {
        return volumeDao.getVolumeByName(novelId, volumeName)
    }

    // Chapter operations
    fun getChaptersByVolumeId(volumeId: Long): Flow<List<Chapter>> {
        return chapterDao.getChaptersByVolumeId(volumeId)
    }

    suspend fun insertChapter(chapter: Chapter): Long {
        return chapterDao.insertChapter(chapter)
    }

    suspend fun updateChapter(chapter: Chapter) {
        chapterDao.updateChapter(chapter)
    }

    suspend fun deleteChapter(chapter: Chapter) {
        chapterDao.deleteChapter(chapter)
    }

    suspend fun getChapterById(id: Long): Chapter? {
        return chapterDao.getChapterById(id)
    }

    suspend fun getChapterByName(volumeId: Long, chapterName: String): Chapter? {
        return chapterDao.getChapterByName(volumeId, chapterName)
    }
}